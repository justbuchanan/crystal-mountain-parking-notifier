...
parser = argparse.ArgumentParser(
    prog='parkbot',
    description=
    'Check the crystal mountain parking website for availability for a given day'
)
...
cal_entries = driver.find_elements(
    By.CSS_SELECTOR, f'#calendar_{args.year}-{args.month:02d} .calendar_day')
cal_entries = [entry for entry in cal_entries if entry.text == str(args.day)]

if len(cal_entries) != 1:
    print(
        f"unexpected html. found {len(cal)} matching calendar entries, wanted 1"
    )
    exit(1)
cal = cal_entries[0]
